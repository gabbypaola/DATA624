...
The relationship between the variables are reviewed using a correlation plot in order to detect multicolliniarity within the training data set. Variables with correlation coefficients greater than 0.90 have been removed from the plot using `findCorrelation`^[https://www.rdocumentation.org/packages/caret/versions/6.0-92/topics/findCorrelation].
...
Pre-processing is needed based on the distributions and missing values noted in the training data set for both linear and non-linear models. Random Forest models do not require pre-processing as in the cases of linear and non-linear models.

```{r}
#pre-process for linear and non-linear
