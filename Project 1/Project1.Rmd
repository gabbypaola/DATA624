...
Using `summary`, there is also one NA in the `KWH` column, and a potential outlier in the minimum value. 
...
Next we clean the data to adjust the variable type of `YYYY-MM` to date type and call it `Month` and create a `tsibble` object from the `tibble`.

```{r}
#change variable type and to tsibble
power <- power_raw %>% 
