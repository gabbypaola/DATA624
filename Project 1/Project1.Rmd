...
Based on the above, clean up is required before converting the tibble to a tsibble object for forecasting.
...
After inspecting the structure of our `power_data`, it looks like there are three columns, one of which is irrelevant (`CaseSequence`) to our forecast of power utilization. The `YYYY-MM` column will need to be converted to a date type in order to be used as the index for the required tsibble object. 
